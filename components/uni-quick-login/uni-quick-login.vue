...
<script>
	...
	export default {
		...
		async created() {
			...
			if (
				this.getRoute(1) == '/uni_modules/uni-id-pages/pages/login/login-withoutpwd' && ['weixin', 'apple'].includes(this
					.loginConfig[0])
			) {
					...
					success: async ({
						...
						if (result.code === 0) {
							...
							if (type == 'weixin' && !result.userInfo.nickname) {
								return this.$refs.userProfile.open(result.uid, result.openid)
							}
							// #endif
