		...
		<view v-if="isAudit">
			<div class="grid">
				<div class="grid-item" v-for="(item, j) in alist" :key="j" @tap="goToDetail(item)">
					<div class="top" v-if="item.banner && item.banner[0]"><img class="img" mode="aspectFill" :src="item.banner[0].url" alt="" /></div>
...
export default {
	...
	data() {
		return {
			...
			// 查询字段，多个字段用 , 分割
			field: '_id,name,source{code,icon},radios,total_view_count,banner,is_on_sale,tag_id',
	...
	async onLoad(options) {
		...
		if (options.order) {
			...
			this.where = 'is_on_sale == true'; // 只显示上架的
			this.orderby = 'total_view_count desc'; // 默认播放最多
		}
