...
class service {
	...
	async closeOrder(data = {}) {
		...
		let uniPayInstance = await this.initUniPayInstance(payOrderInfo);
		// console.log('uniPayInstance', uniPayInstance)
		let closeOrderRes = await uniPayInstance.closeOrder({
			outTradeNo: out_trade_no
		});
		// console.log('provider', provider)
		// console.log('closeOrderRes', closeOrderRes)
		// console.log('closeOrderRes.resultCode', closeOrderRes.resultCode)
	...
	async initUniPayInstance(data = {}) {
		...
		let uniPayInstance;
		// console.log('uniPayConifg', uniPayConifg)
		if (provider === "wxpay") {
			// 微信
			if (uniPayConifg.version === 3) {
				try {
